...
public class SearchService {
  ...
  class MembersSearchTool implements ItemSearch<SearchMembersResponse> {
    ...
    public <T> SearchPageGetResponse<T> getEveryItem(Pageable pageable, String memberEmail, ItemSearch<T> searchTool) {
        Member member = validate(memberEmail, new MemberValidation());
        Page<T> pagedItems = searchTool.searchAll(pageable);

        return SearchPageGetResponse.of(String.format("총 %s명의 사용자를 찾았습니다.", pagedItems.getTotalElements()), pagedItems);
    }

    public SearchPageGetResponse<TrackGetResponse> getEveryTrack(Pageable pageable, String memberEmail) {
  ...
  public <T> SearchPageGetResponse<T> findWithKeyword(Pageable pageable, String searchKey, String memberEmail, ItemSearch<T> searchTool) {
    Member member = validate(memberEmail, new MemberValidation());
