...
@Slf4j
public class NewTrackFetch implements NewFetch<NewTrack> {
  ...
  @Override
  public Set<NewTrack> parseIntoEntitiesAllUnique(Set<String> titles) {
    Set<NewTrack> tracks = new HashSet<>();

    for (String title : titles) {
      String[] trackAndAlbumAndArtist = title.split("\n");

      NewTrack track = NewTrack.builder()
