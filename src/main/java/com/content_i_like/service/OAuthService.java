...
public class OAuthService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        UserProfile userProfile = OAuthAttributes.extract(registrationId, attributes);  //registrationId에 따라 유저 정보를 통해 공통된 UserProfile 객체로
        System.out.println("oauthId: "+userProfile.getOauthId()+ ", name: "+userProfile.getName() + ", email: "+userProfile.getEmail());
    ...
    private Member saveOrUpdate(UserProfile userProfile){
        ...
        if(member.isEmpty()){                                                               //회원가입된 적이 없다면
