...
public class SecurityConfig {
  ...
  private final String[] AUTHORIZATION = {"/api/v1/**", "/**", "http://www.thymeleaf.org/**",
      "http://www.ultraq.net.nz/**"};
  private final String[] TEST_URL = {"/api/v1/hello/**", "/api/v1/member/join",
      "/api/v1/member/login", "/api/v1/member/passwd/find_pw"
      , "/api/v1/test/**", "/upload", "/member/**", "/inquiry/**", "recommends/**", "/follow/**",
      "/search/tracks/**", "/notifications/**"};
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    http
        ...
        .userService(
            customOAuth2UserService);     // OAuth2 로그인 성공 시, 후작업을 진행할 UserService 인터페이스 구현체 등록;
    return http.build();
  }
}